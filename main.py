    ...
    if uploaded_file is not None:
        ...
        invert = cv2.bitwise_not(dst_sketch)
       	...
       	if option == "Pencil Sketch":
       		...
       		img = plt.imshow(invert, cmap = option+'s'), plt.axis('off'), plt.title('Pencil-Sketch in '+option, size=20)
       		st.pyplot(fig)
       		#plt.imsave(img,fig)
       	...
       	if option == "Color Pencil":
       		fig = plt.figure(figsize = (12,5))
       		plt.imshow(cv2.cvtColor(dst_color_sketch, cv2.COLOR_BGR2RGB)), plt.axis('off'), plt.title('Color Pencil-Sketch', size=20)
       		st.pyplot(fig)
       		if st.button('Download'):
        ...
        if option == "Water Color":
       		fig = plt.figure(figsize = (12,5))
       		plt.imshow(cv2.cvtColor(dst_water_color, cv2.COLOR_BGR2RGB)), plt.axis('off'), plt.title('Water Color Image', size=20)
       		st.pyplot(fig)
       		if st.button('Download'):
